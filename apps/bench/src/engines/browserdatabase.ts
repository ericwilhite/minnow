...
export const browserDatabaseDriver: EngineDriver = {
  id: "browserdatabase",
  ...
  async loadDataset(context: LoadContext): Promise<EngineMaterialization> {
    ...
    const database = new BrowserDatabase(store, databaseOptions(record));
    let insertMs = 0;
    try {
      ...
      for (const entity of entities) {
        ...
        for (let start = 0; start < entityRows; start += BATCH_ROWS) {
          ...
          context.report(`BrowserDatabase · ${entity.name}`, completedRows);
        }
      }
      const orderRows =
      ...
      if (counted.rows[0]?.row_count !== orderRows) {
        throw new Error(
          `BrowserDatabase verification failed: expected ${String(orderRows)} orders, found ${String(counted.rows[0]?.row_count)}`,
        );
      }
      return {
        engine: "browserdatabase",
        status: "ready",
        storageName: name,
        version: "workspace",
