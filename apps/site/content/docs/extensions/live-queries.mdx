...
`@minnowdb/core/live` exports `LiveQuerySource`, `LiveQueryManager`, and
`createLiveQueryManager` for that purpose.
The adapter continues to execute its own query, so name mapping and other result transforms are not
bypassed. A library-specific composition helper may call the wrapper, but is not required.
