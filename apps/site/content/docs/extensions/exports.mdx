...
Each page has the same `columns`, `columnDomains`, and `rows` shape as `query()`. Empty results
yield one empty page, so exporters can still write a header and inspect domains. Breaking out of
the loop, calling `return()`, or aborting the optional `signal` releases the scan and its snapshot
lease.
