...
- **Library browser tests** exercise IndexedDB, OPFS, and transaction behavior that a Node substitute
  ...
  They also run the SQL engine itself where it ships: the committed SQLLogicTest profile executes
...
the driver uses it. A run that meets an unresponsive store anyway records it in
`transientsAccepted` and stops there, with `interactions` counting the steps it judged; the engine
bounding the wait and saying so is the accepted outcome, since nothing further can be driven
through a store the browser has wedged.
...
connection, which is why the Node driver opens one per connection for IndexedDB and OPFS.
...
capability, while IndexedDB cases still run. Worker startup errors and probe timeouts remain
failures. Contention correctness tests allow queued durable writes a 30-second RPC deadline;
