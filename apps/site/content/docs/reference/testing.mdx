...
result summaries. Failure traces are retained, and CI uploads the report and `test-results/`
when a browser gate fails. A console log alone is not a replay artifact.
...
counts, `RETURNING`, and the resulting keyed table state. Nondeterministic `ANY_VALUE`,
nonportable features, and remaining DDL have explicit acceptance categories; the report does not
count acceptance alone as a result comparison. The category totals must cover the entire matrix.
