...
for (const table of catalog.tables) {
  ...
  table.columns; // { id, name, type, integer?, nullable, defaultValue?, enumValues?, isAutoIncrementing }
...
// each carries: id, an Annex F feature or "minnow", an example, and the error it raises
```

There are 201 entries, with 182 supported. The engine's tests read this same file, so a change to
...
| Gap                             | Note                                                      |
| ------------------------------- | --------------------------------------------------------- |
| correlated non-equi scalar/`IN` | range correlation is currently for `EXISTS`/`NOT EXISTS`  |
| correlated `NOT IN`             | use `NOT EXISTS`                                          |
| correlated `EXISTS` below `OR`  | keep it as a top-level `WHERE` conjunct                   |
| `LATERAL` sources               | no operator re-executes a source per row of its left side |
| exact `NUMERIC` / `DECIMAL`     | use safe integer minor units or canonical decimal text    |
| secondary and composite keys    | one unique key column is the only keyed address today     |
| savepoints                      | only the outer transaction can roll back                  |
| upsert `DO UPDATE ... WHERE`    | use `CASE` when retaining the old value is equivalent     |
