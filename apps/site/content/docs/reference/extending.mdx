...
official [`@minnowdb/kysely`](/docs/adapters/kysely) package — use the public APIs on this page.

The Kysely adapter ships separately and uses only these APIs. That keeps this page grounded in a
real adapter instead of a set of hooks that have never been used outside the engine.
...
for (const table of catalog.tables) {
  ...
  table.foreignKeys; // scalar column fields plus columns?/parentColumns? for composite keys
...
| correlated scalar/`IN` range predicates   | equality correlation only; range correlation is available for `EXISTS` |
| correlated `EXISTS` below `OR`            | keep correlation as a top-level conjunct                               |
| correlated `LATERAL` grouping/order/limit | use the supported equality-correlated projection/join shape            |
