...
- Unsupported: keyless `UPDATE`/`DELETE`, `GRANT`, configurable transaction isolation,
  correlated LATERAL grouping/order/limit, correlated JSON_TABLE documents, enum evolution, and
  sequence options. Check each supported entry's note before assuming every PostgreSQL variant.
```
