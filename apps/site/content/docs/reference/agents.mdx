...
- Supported PostgreSQL forms include exact NUMERIC/DECIMAL, JSON/JSONB, UUID, arrays, DATE, TIME,
  ...
  NOT IN, range-correlated LATERAL, STRING_AGG, SIMILAR TO, COLLATE, and constant JSON_TABLE.
- Unsupported: keyless `UPDATE`/`DELETE`, `GRANT`, configurable transaction isolation,
  nested correlated `ANY`/`ALL`, correlated LATERAL grouping/order/limit, correlated JSON_TABLE
  documents, enum evolution, and sequence options. Check each supported entry's note before
  assuming every PostgreSQL variant.
```
