...
- Treat Minnow's fixed SQL safety limits as input validation, not tuning: one statement is capped
  at 1,048,576 characters, 4,096 tokens and parameter positions, and 128 syntax levels. Pattern,
...
- `INTEGER`, `BIGINT`, and `SMALLINT` accept only JavaScript safe integers. `NUMERIC` and
  ...
  is intended. JSON/JSONB, UUID, arrays, `DATE`, `TIME`, intervals, and enum values also return
  strings at the JavaScript boundary. `DATE` is canonical `YYYY-MM-DD` calendar text, not a
  JavaScript `Date`.
...
- Supported PostgreSQL forms include exact NUMERIC/DECIMAL, JSON/JSONB, UUID, arrays, DATE, TIME,
  ...
  range-correlated LATERAL, ordered STRING_AGG/JSON_ARRAYAGG, SIMILAR TO, COLLATE, and constant
  JSON_TABLE.
