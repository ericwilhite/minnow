...
- Unsupported: keyless `UPDATE`/`DELETE`, `GRANT`, `SERIALIZABLE` isolation, DDL inside
  `BEGIN … COMMIT`, correlated LATERAL grouping/order/limit, correlated JSON_TABLE documents,
  arrays beyond a JSON-text literal (no subscripts, `ANY`/`ALL` over array values, `unnest`,
  `array_agg`, `generate_series`), `||` on array or JSONB operands and the `#>`/`@>`/`?`
  operators, `jsonb_set`/`jsonb_typeof`/`json_object_agg`, interval-valued arithmetic such as
  interval-plus-interval or timestamp-minus-timestamp, `AT TIME ZONE`, `ALTER TABLE` beyond
  ADD/DROP COLUMN, partial and expression indexes, `ON CONFLICT` on a non-key UNIQUE column,
  schemas and `information_schema`, `EXPLAIN`/`VACUUM`/`ANALYZE`, ordered-set aggregates,
  bitwise operators, `bytea`, enum evolution, sequence options, `RIGHT JOIN` or `FULL JOIN`
  beside another join or combined with `SELECT *`, `FULL JOIN` without a single equality `ON` or
  with grouping, `DISTINCT`, or window functions, numeric `RANGE` frame offsets (use `ROWS`),
