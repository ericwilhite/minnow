...
- For typed Kysely live queries, create one `createKyselyLiveQueries({ driver })` manager and wrap
  ...
  One subscription per component is fine: unchanged rows keep their object identity across
  snapshots, so memoize list items on the row and use `useLiveSelector` for a derived value.
...
- Unsupported: keyless `UPDATE`/`DELETE`, `GRANT`, `SERIALIZABLE` isolation, `LOCK TABLE`, DDL
  ...
  grouping/order/limit, correlated JSON_TABLE documents, arrays beyond a JSON-text literal (no
  subscripts, `ANY`/`ALL` over array values, `ARRAY(SELECT …)`, `unnest`, `array_agg`,
  ...
  `FULL JOIN` beside another join or combined with `SELECT *`, `FULL JOIN` without a single
  equality `ON` or with grouping, `DISTINCT`, or window functions, numeric `RANGE` frame offsets
  (use `ROWS`), `DISTINCT` window aggregates, window functions outside the select list, and
