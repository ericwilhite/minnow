...
- `CREATE INDEX name ON table(column)` creates a durable non-unique single-column accelerator for
  equality, range, and `IN` filters; `DROP INDEX name` removes it. Composite and `UNIQUE` indexes
  are rejected, and an index does not currently satisfy `ORDER BY`.
...
- Not supported, and what to write instead: `LATERAL` (rewrite as a join), `LISTAGG`
  ...
  `SET TRANSACTION`, composite or `UNIQUE` secondary indexes, multiple or composite unique keys,
  composite foreign keys, savepoints, correlated `NOT IN` (use `NOT EXISTS`), non-equality
