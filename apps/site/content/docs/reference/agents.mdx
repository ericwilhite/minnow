...
- On teardown, `await db.close()` before `store.close()` for a direct engine. For a worker,
  ...
  Worker calls have a 60-second default `requestTimeoutMs`; close has a five-second default
...
- An idle SQL transaction rolls back and rejects subsequent statements with
  `TransactionExpiredError` until explicit `ROLLBACK` or `BEGIN`. Keep transaction callbacks
...
- OPFS coordination exhaustion and queue overload throw `OpfsCoordinationError` from
  ...
  `OpfsUncertainOutcomeError` needs reconciliation because its mutation may have committed.
- OPFS deletion requires Web Locks and all connections closed. `OpfsDatabaseInUseError` refuses
  deletion while a current client is open. Close pre-0.9.0 clients too: they do not participate
  in the deletion lock protocol.
```
