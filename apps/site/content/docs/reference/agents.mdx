...
- `INTEGER`, `BIGINT`, and `SMALLINT` accept only JavaScript safe integers. `NUMERIC` and
  ...
  constant or constant-arithmetic result Float64 cannot hold exactly (24-digit decimals, integers
  beyond 2^53) comes back as a decimal string rather than a rounded number or an error.
...
- Unsupported: keyless `UPDATE`/`DELETE`, `GRANT`, configurable transaction isolation,
  correlated LATERAL grouping/order/limit, correlated JSON_TABLE documents, enum evolution,
  sequence options, `RIGHT JOIN` beside another join or `SELECT *`, `FULL JOIN` with grouping,
