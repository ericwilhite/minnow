...
The short version: data lives in compressed columnar blocks inside IndexedDB or OPFS, reads see consistent
snapshots (MVCC), and queries run through a vectorized executor. This page covers the choices and
the reasons; the code is the full record.
...
- **Notifications are hints.** BroadcastChannel announces that a new version exists, but every tab
  ...
  result. [Live queries](/docs/sql) are built on this.
...
- **Garbage collection is reachability.** Whatever the current manifest, unexpired leases, live
  ...
  atomically, so nothing is revived after it's gone. Once a committed transaction's manifest is
  pruned and no segment or unfinished compaction names it, its record is collected too. Aborted
  journals follow their pending artifacts. Expired leases, abandoned spill owners, old manifest
  tombstones, and finished job history are swept as part of the same maintenance cycle; ordinary
  use leaves no permanent per-operation metadata residue.
