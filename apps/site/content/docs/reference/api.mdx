...
| ---------------------------------------- | --------------------------------------------------------------- |
| `@minnowdb/core`                         | Schema DSL, typed client and builders, engine, live queries     |
...
| `.autoIncrement() / .default(value \| fn)`                                                               | Generated values: a persistent cross-tab counter for number unique keys; literal / `"now"` defaults filled engine-side on every write path; function defaults called by the typed facade before its INSERT is sent. |
| `schema(tables, { views })`                                                                              | Bundles tables and views into a `SchemaDefinition` for `migrate()` and the typed client.                                                                                                                            |
...
| Export                                                      | Description                                                                                                                                                                                                               |
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `createMinnow<DB>(driver, { schema })`                      | Builds the typed client. The standard form passes a named `interface DB extends InferDatabase<typeof appSchema> {}` so tooling prints `Minnow<DB>`; omitting the type argument infers `DB` from the schema value instead. |
| `class Minnow<DB>`                                          | The typed client itself; wraps a `MinnowDatabase` or `MinnowDatabaseClient` (any `DslDriver`).                                                                                                                            |
...
| `.execute(sqlOrFragment, params?)`                          | Runs one query, mutation, DDL, trigger, or transaction statement. The result's `kind` field says what happened.                                                                                                           |
...
| `.close()`                                                  | Closes the typed client's shared live-query set. Close the driver separately when the application owns it.                                                                                                                |
| `.driver`                                                   | The `MinnowDatabase` or `MinnowDatabaseClient` behind the typed client. Application code should keep its own reference when it already has one.                                                                           |
| `MinnowOptions`                                             | Typed client options: the schema, plus `live: { channelName?, pollIntervalMs? }` defaults for `.live()`.                                                                                                                  |
| `DslDriver, DslWriteSession, DriverLiveSet, DslLiveOptions` | The small driver interfaces shared by the database and worker client.                                                                                                                                                     |
...
| `.toSQL()`                                                                   | Parameterized SQL for insert, update, and delete builders.                                                                      |
...
From `@minnowdb/core`. The low-level asynchronous engine the typed client drives. See
...
[Typed client](#typed-client) above and [Schema & migrations](/docs/schema).
