...
| `class LiveQuerySet`                                                | Low-level SQL/plan set from `database.liveQueries()`: `subscribe`, observer-only `observe` (optionally `suppressUnchanged`), `refresh`, stats, and `close`. |
...
| `LiveQuerySetOptions / LiveQueryStats / LiveQueryInput`             | Hint/poll/`sharedResults` configuration, execution counters, and low-level query inputs.                                                                    |
| `LiveQueryHost / LiveQueryExecuteContext`                           | The probe, manifest, dependency, and execution contract a `LiveQuerySet` runs over.                                                                         |
...
`MinnowDatabaseOptions` covers the schema the database is typed against, compression, block size,
write retries, query spilling, idle SQL transactions, caching, automatic full-text indexing, and
background maintenance. The full option
...
| `WorkerStoreFactory, WorkerStoreOptions`    | The store factory signature `(descriptor, options) => BlockStore \| Promise<BlockStore>` and its options (`indexedDB?`).                                                                                                 |
| `StoreDescriptor`                           | `{ kind: "indexeddb", name, durability?, uniqueKeyCacheBytes? } \| { kind: "opfs", name, … } \| { kind: "memory" }` — the cloneable store config. Omitted, it defaults to `{ kind: "indexeddb", name: "minnow" }`.       |
...
code; each refuses an init frame for another store kind. See
[per-store worker entries](/docs/engine/workers#per-store-worker-entries).
...
written by the locked block-format 2, snapshot-format 1, IndexedDB-schema 1, and OPFS-layout 5
writers are separate format contracts and cannot be reinterpreted in place.
