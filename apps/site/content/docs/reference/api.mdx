...
| ------------------------------------------------ | --------------------------------------------------------------- |
| `@minnowdb/core`                                 | SQL engine, schema DSL, catalog, migrations, and errors         |
| `@minnowdb/core/client`                          | Main-thread worker client                                       |
| `@minnowdb/core/query`                           | SQL compiler, standalone executor, binding, and plan inspection |
| `@minnowdb/core/live`                            | Typed, keyed, and low-level live queries                        |
| `@minnowdb/core/schema`                          | Schema DSL, typed tables, and migrations without the engine     |
| `@minnowdb/core/schema-wire`                     | Schema serialization for workers and adapter tooling            |
| `@minnowdb/core/worker`                          | Ready-made worker entry (side-effect import)                    |
| `@minnowdb/core/worker-host`                     | Manual host for custom worker entry points                      |
...
| `@minnowdb/core/storage/{indexeddb,opfs,memory}` | One storage adapter without the other adapters                  |
| `@minnowdb/core/storage/contracts`               | Storage interfaces, records, limits, and errors                 |
| `@minnowdb/core/storage/snapshots`               | Portable snapshot framing and streaming helpers                 |
| `@minnowdb/core/storage/persistence`             | Browser origin-persistence policy                               |
...
| `@minnowdb/react`                                | React external-store hook for typed live queries                |
...
| `column.integer() / numeric() / json() / jsonb() / uuid() / date() / time() / interval() / array()`      | Builders retaining the SQL engine's domain metadata and exact boundary types.                                                                                                     |
...
- `attachLifecycleFlush(writerProxy, options)` — requests flushes on `visibilitychange` /
  `pagehide`.
- `QueryOptions` and `QueryCursorOptions` — including execution memory, spill, batch, and abort
  configuration;
  ...
  aliased JSON constructors; `WriteMetrics` appears on every batch result.
...
| Mirrored API                                                                                                  | Queries, pull-driven query cursors, writes, migrations, reads, live queries, snapshots, and maintenance. Direct `createView`, `dropView`, and `buildFtsIndex` helpers are not exposed; migrations manage views and search indexes build automatically. |
