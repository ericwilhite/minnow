...
Then the gate, the tag, and the publish:

```bash
npm run check:release            # the local gate, the performance gate, all browser suites
git commit -am "Release core 0.2.0" && git tag @minnowdb/core@0.2.0
npm publish --workspace @minnowdb/core --access public
```

A major moves everything at once, so it is tagged `v2.0.0` and published with
`npm publish --workspaces`. `npm run version:check` proves the workspace still agrees with itself
— matching majors, ranges that span them, and a docs version that matches the engine — and runs
as the first step of `npm run check`, so a hand-edited version fails before anything is published.
