...
The client and devtools console use the engine's public APIs. A change that stops those APIs from
working is a major change, so each package depends on its siblings
