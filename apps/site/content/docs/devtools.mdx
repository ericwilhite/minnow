...
That confirmation step covers every statement that can change the database: `INSERT`, `UPDATE`, `DELETE`, `MERGE`, schema
DDL, index and trigger DDL, and transaction control. Destructive schema operations say what will
be removed. Successful DDL refreshes the rail and completion automatically, so a new table,
column, or index is available without pressing reload.
