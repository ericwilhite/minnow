...
The launcher below is the real thing, in its production shape: a database running in a web worker
(the same [worker client](/docs/client/workers) an app ships), loaded with a generated five-table
commerce dataset — 165,808 rows of regions, customers, products, orders, and order items. The
panel floats over these docs exactly as it would float over your app, and the title-bar badge
reads **worker** because the queries genuinely leave the page. For the same panel rendered inline
against a small in-memory database, see the [playground](/playground).
...
- **worker** or **main thread** — a database built in the page runs queries on the main thread, so
  a slow one will freeze it. The [worker client](/docs/client/workers) does not.
...
panel, in flow, sized by its container. The [playground](/playground) is exactly this,
