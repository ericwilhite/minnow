---
title: Devtools
description: An embeddable SQL console for your database — a floating panel in dev, an inline console in docs pages.
---
...
nullability, and which one is the unique key. SQL-declared indexes follow the columns with their
ordered keys, descending directions, uniqueness, and build health. Knowing what a column or index
is called is as useful for writing a statement as it is for browsing a table, so the rail never
goes away.

What clicking does follows the tab you are on:

- On **Query**, a table, column, or index name is inserted at the caret — `orders`,
  `orders.total`, or `orders_by_total` — spaced from whatever precedes it.
...
when it is a datetime (the explorer deliberately renders its filter and cursor values at day
precision). Both give the same rows; one just gets slower the further in you go.

**Counting.** `COUNT(*)` scans the whole table, so it runs alongside the first page rather than
delaying it. While it runs, the status bar says how many rows are loaded.
...
After a write the row is read again rather than patched in place, so the grid shows the committed
value.
...
when you look at it, and running a query returns you to the rows. Non-query statements say that a
SELECT is required instead of passing DDL to the query planner.

The Query/Data and Rows/Plan tab bars use the standard arrow, Home, and End keys. The editor
divider is keyboard-resizable with Up and Down. In the data grid, arrow keys move between visible
cells, Space selects a row, and Enter starts editing a cell when editing is available.
