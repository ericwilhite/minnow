...
The [playground](/playground) is this panel, in its production shape: a database running in a web
worker (the same [worker client](/docs/client/workers) an app ships) over a seven-table retail
dataset generated in your browser and kept in IndexedDB. The title-bar badge reads **worker**
because the queries genuinely leave the page.
