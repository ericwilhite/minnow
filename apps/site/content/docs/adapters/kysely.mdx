...
shape: a hand-written `Kysely<DB>` interface whose column is the object type, as in the example
above. A schema-derived `createKysely` database types JSON columns as their lossless string
boundary (or `MinnowJsonValue` with decoding), which offers no member names to traverse — write
those reads with `sql<T>` and `JSON_VALUE`, or declare an interface for that table. Two
accuracy notes on Kysely's inferred leaf types: a `->` result matches its inferred object type
at runtime only with `resultDecoding: { json: "parse" }` (otherwise it is JSON text), and a
`->>` result is always text at runtime even where the declared member is a number or array —
treat it as a string, exactly as the comparison above does.
...
- Kysely can generate PostgreSQL features outside Minnow's profile. The adapter's compiler
  refuses the forms Kysely's builders can produce before execution, naming the feature and an
  alternative: `DISTINCT ON`, `UPDATE ... FROM`, `DELETE ... USING`, `ON CONFLICT` with a
  constraint or index-expression target (name the unique key's columns instead), row-locking
  clauses such as `.forUpdate()`, schema-qualified names including
  `WithSchemaPlugin`, `ALTER TABLE ... RENAME`, and an empty `IN ()` list (add Kysely's
  `HandleEmptyInListsPlugin`). Anything else outside the profile is rejected explicitly by the
  engine; check the executable [feature matrix](/docs/sql/feature-matrix).
...
Adapter tests compile and execute DDL, inserts, queries, `RETURNING`, commits, rollbacks,
nested savepoints, `MERGE`, connection serialization around open transactions, and
