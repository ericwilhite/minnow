...
The adapter compiles these builders to ordinary PostgreSQL SQL and passes it to Minnow's shared
planner; it does not maintain a shallower subquery implementation of its own. Kysely has no
first-class `ALL (subquery)` builder or correlated `selectNoFrom` on an expression builder. Those
...
- Kysely can generate SQL outside Minnow's profile — PostgreSQL features Minnow lacks, and the
  ...
  - Queries: `DISTINCT ON`, row-locking clauses such as `.forUpdate()`, SQL `EXPLAIN` (use the
    ...
    `.outerApply()`, and an empty `IN ()` list (add Kysely's `HandleEmptyInListsPlugin`).
  - Mutations: `UPDATE ... FROM`, `DELETE ... USING`, MySQL `ORDER BY`/`LIMIT` on updates and
    deletes, `replaceInto()`, `onDuplicateKeyUpdate()`, SQLite `orIgnore()`/`orAbort()` and
    friends, `ON CONFLICT` with a constraint or index-expression target (name the unique key's
    columns instead), `MERGE ... THEN DO NOTHING` (narrow the `WHEN` condition instead), and
    data-modifying CTEs (run the mutation first, then query).
  - DDL: schema-qualified names including `WithSchemaPlugin`, `CREATE`/`DROP SCHEMA`,
    ...
    alterations), serial/identity/`autoIncrement()` column DDL (declare it in Minnow's schema
    DSL, or default to `nextval()` of a `CREATE SEQUENCE` sequence), temporary tables and views,
    materialized views, view column lists and `CREATE VIEW IF NOT EXISTS`, non-stored generated
    columns, `DROP`/`ALTER TYPE`, index access methods (`.using()`), partial and expression
    indexes, and `NULLS NOT DISTINCT`.
