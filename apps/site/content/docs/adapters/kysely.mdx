...
- Kysely can generate PostgreSQL features outside Minnow's profile. The adapter's compiler
  ...
  constraint or index-expression target (name the unique key's columns instead), the `->` and
  `->>` JSON operators (use the SQL-standard `JSON_VALUE` and `JSON_QUERY` forms through
  Kysely's `sql` tag, or the JSON helpers), row-locking clauses such as `.forUpdate()`,
  schema-qualified names including
