...
also include the empty-input NULL. Use `eb.fn.coalesce(aggregate, eb.val(fallback))` when the query
...
`number`, exact numeric and string-backed logical domains infer `string`, boolean infers `boolean`,
and date/time-stamp targets infer `Date`.
