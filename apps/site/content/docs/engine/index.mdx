...
| `autoCompact`      | `true`        | Whether fragmented tables and delta histories are folded in the background; see [compaction](/docs/storage/maintenance).                                                                                          |
| `autoCollect`      | `autoCompact` | Whether superseded blocks and old versions are collected in the background; see [garbage collection](/docs/storage/maintenance#garbage-collection).                                                               |
| `compaction`       | `{}`          | Defaults for every fold this database plans, background ones included: `partitionRows` (16,384) sizes a keyed table's level-one partitions. See [partitioned folds](/docs/storage/maintenance#partitioned-folds). |
