...
compile. See [typed batch writes](/docs/schema#typed-batch-writes).
...
`flush()` joins earlier queued `add()` calls and any in-flight flush, then commits the accepted
...
| ----------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `schema`                      | none     | The `schema([...])` declaration this database is typed against. Every batch method is then keyed by table name (see [typed batch writes](/docs/schema#typed-batch-writes)), and a bare `migrate()` applies it.    |
...
| `coordinateWrites`            | `true`   | Coordinate autocommit writers locally and across tabs with Web Locks when available. Disable only for custom scheduling or optimistic contention.                                                                 |
...
block sizing, `coordinateWrites`, retry and buffer limits, `executionMemoryBudgetBytes`, `transactionOwnerLeaseMs`,
...
`db.close()` rejects new work, cancels open write/snapshot callbacks and an open `BEGIN`, closes cursors and export iterators, stops live-query polling and background scheduling,
