...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------------------------------ | ----------------- |
| `unknown-outcome` | `DatabaseWorkerOutcomeUnknownError`, `OpfsUncertainOutcomeError`, `StorageUnresponsiveError` — all extend `UnknownOutcomeError`                                                                                                                         | **yes**            | after reconciling                                | see below         |
...
`StorageUnresponsiveError` means the database answered nothing at all for thirty seconds while
the engine had work outstanding. Nothing in IndexedDB can be cancelled or timed out, so this
deadline is what stands between a wedged browser and a call that never returns; any storage event
resets it, so work queued behind a genuinely long transaction is never mistaken for a wedge.

The known cause is a worker killed with a write in flight — `worker.terminate()` from your own
code, or the browser killing it. Safari and other WebKit browsers keep the dead worker's
IndexedDB connection, and its unfinished transaction, registered until the document that created
the worker goes away. While it is registered, every connection to that database blocks, including
connections opened afterwards in other tabs.

**Reload the page.** `reopen()` is not enough: a new worker in the same document opens the same
wedged database. Reloading, or closing the tab, releases the dead connection immediately. Let the
engine terminate workers for you — `close({ terminateWorker: true })` disposes the store first —
and treat a write that was in flight as an unknown outcome.
