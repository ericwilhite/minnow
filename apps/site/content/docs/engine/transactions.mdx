...
Schema changes take turns too. A migration or DDL statement waits for the writers ahead of it and
publishes while none is mid-transaction, so a cooperating writer never has its scope invalidated
by a schema change; the schema epoch check below remains for writers that do not take turns. A
write queued behind a migration that removes the table it needs fails against the new schema,
as it should — that is an application error, not contention.
