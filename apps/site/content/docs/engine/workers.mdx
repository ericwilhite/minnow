...
- **Query controls stay local.** `signal` and `onStats` are functions/objects that cannot be
  ...
  (6 since core 0.7.10); a mismatch fails every call loudly with "Unsupported protocol version"
...
format the generic worker is about 1.5 MB minified (419 KB gzipped), against about 1.2 MB
(340 KB gzipped) for the IndexedDB store alone.
