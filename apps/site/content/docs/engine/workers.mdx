...
session calls run in order before commit. Snapshot queries use the live worker scope, so an expired
or closed scope cannot silently read through a replacement lease.
