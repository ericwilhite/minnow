...
wait continues. Without a handler each one is written to `console.error` on the main thread.
...
[write scopes](/docs/engine/transactions#atomic-writes)). Closing a client rejects every call it
