...
startup. Increase it for intentionally long imports or queries. Expiry closes the connection to
...
format the generic worker is about 1.5 MB minified (422 KB gzipped), against about 1.2 MB
(343 KB gzipped) for the IndexedDB store alone.
...
`opfs`, or `memory` — omitted, it defaults to `{ kind: "indexeddb", name: "minnow" }`) plus `compression`, `rowsPerBlock`, `targetBlockBytes`, `maxCommitRetries`, `coordinateWrites`,
