...
- **Query controls stay local.** `signal` and `onStats` are functions/objects that cannot be
  ...
  same controls as the direct engine. Cancellation requires protocol version 3; deploy the client
  and worker from the same package build so a cached older worker cannot outlive its client.
