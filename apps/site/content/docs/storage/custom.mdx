...
`WriteConflictError`, `TransactionRecordConflictError`, `UniqueKeyConflictError`,
...
`getQueryCatalogState` returns the named tables plus only the segments and transactions reachable
from the current manifest version returned in the same atomic read. Include that manifest's exact
block IDs too, so the engine can pin the already-resolved snapshot instead of reading it again. Do
not return staged or superseded history: current-version query preparation must not grow with
retained time-travel history. Explicit historical reads use the paged segment and transaction
methods instead.
