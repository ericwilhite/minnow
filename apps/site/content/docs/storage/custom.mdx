...
atomically. `removePrunedManifestRecords` then deletes only tombstones no readable delta chain
needs **and whose garbage blocks are all gone**. The tombstone is the durable discovery record
between bounded collection passes, so deleting it early can orphan the rest of a large backlog. A
