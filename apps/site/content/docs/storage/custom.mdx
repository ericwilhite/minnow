...
manifests, segments, blocks, then committed transactions; a transaction can disappear only after
its manifest is pruned and no segment or unfinished compaction names it. Adapters must apply that
whole step atomically. A file-backed adapter may delete the immutable block payload after logging
