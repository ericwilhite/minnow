...
Catalog cleanup follows the same rule. When `updateTable` removes a column, that operation also
removes the column from `ftsColumns` and deletes all of its full-text base chunks and commit
deltas. `removeFtsColumn` provides the same cleanup directly. A dropped column must not leave an
index growing invisibly for the rest of a long-running database's life.
