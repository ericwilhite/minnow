...
flushes. It is an explicit performance choice for rebuildable or remotely synchronized data: a
machine or storage-device power loss may lose an acknowledged suffix of commits.
...
clearing site data, so export irreplaceable data or synchronize it elsewhere too.
...
IndexedDB schema 1 is Minnow's first and only initial native layout. There are no pre-contract
schemas or compatibility branches. Future schema changes must add one
