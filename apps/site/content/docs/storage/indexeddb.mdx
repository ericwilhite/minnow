...
byte-for-byte logical state. A frozen database of every stable schema is opened, migrated, written,
and checked in CI, and the migration registry refuses to load if a schema bump omitted or
reordered a step.
