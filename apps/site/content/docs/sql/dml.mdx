...
Any query expression can feed the insert — a `WITH`, a `UNION`, a parenthesized member — and
`ON CONFLICT` applies to the rows it produces exactly as to a `VALUES` list. A `VALUES` row may
carry scalar subqueries, evaluated when the statement runs:
...
Both statements take PostgreSQL's table alias (`UPDATE customers AS c`, `DELETE FROM orders o`),
...
`TRUNCATE [TABLE] name` removes every row of one table, the same as an
...
A string constant or bound parameter written into a datetime, number, or boolean column is read
...
works too, with the semantics it has in a `SELECT`: `RETURNING product_id, list_price * 1.05 AS
next_price, UPPER(name) AS label` reads the post-update row for `UPDATE`, the written row for
...
`SERIALIZABLE` is refused rather than silently downgraded. Session settings — `SET name TO
value`, `SET LOCAL …`, `SET TRANSACTION …`, `RESET name` — are accepted and ignored, since an
