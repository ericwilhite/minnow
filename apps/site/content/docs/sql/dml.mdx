...
Prefer a typed builder to SQL strings? [Mutations](/docs/client/writes) covers the same ground
...
can read the row's current values, as `total - ?` does above.
...
`ON CONFLICT` is a PostgreSQL-compatible extension; SQL:2023 standardizes `MERGE` instead.
...
When every non-key column should be replaced, Minnow also accepts the concise whole-row form used
by the typed client's `orReplace()`:
...
windows, and subqueries are not allowed in the assignment. A `WHERE` predicate after the
`DO UPDATE SET` list is not supported yet; use `CASE` to retain the stored value when that is
equivalent.
