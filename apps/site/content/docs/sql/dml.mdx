...
can read the row's current values, as `total - $1` does above. Subquery predicates use the same
resolution and decorrelation pipeline as `SELECT`, including `IN (SELECT ...)`, `NOT IN (SELECT
...)`, and correlated `EXISTS` / `NOT EXISTS`. A mutation that still contains a materialized
subquery takes the general selection executor instead of the streamed key-only path.
...
`DO NOTHING` is also available; a later duplicate key in the same `VALUES` list is skipped after
the first proposal is retained. `EXCLUDED` refers to the row that would have been inserted,
...
Trigger names are global within a database. Creation reserves the name and stores an immutable
trigger identity in the same atomic catalog mutation, so two tabs cannot both create the same
name and an old `DROP TRIGGER` cannot delete a later same-name replacement. Minnow resolves every
NEW/OLD binding, body target, assignment, predicate, required target column, and default before
admitting the trigger. Views are not writable trigger owners or targets. A later column/default
change that would invalidate a stored body is refused; a write already staged under the old
trigger catalog gets a schema conflict instead of publishing missing or obsolete effects.
