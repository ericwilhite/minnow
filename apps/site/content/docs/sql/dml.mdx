...
plan is compiled: `CURRENT_DATE`, `CURRENT_TIMESTAMP`, and `LOCALTIME` are stable across all rows
of one statement, while `random()`, `gen_random_uuid()`, and `nextval(...)` run for each written
expression.
