...
- a correlated scalar's inner block supports aggregate expressions or an ordinary single row,
  with per-probe ordering and limits, but not its own `GROUP BY` or `HAVING`.
- `LATERAL` supports equality and range-correlated projections and joins, without correlated grouping,
  ordering, or `LIMIT`.
