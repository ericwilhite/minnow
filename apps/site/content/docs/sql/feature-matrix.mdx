...
For the upstream corpus, differential oracles, fault simulation, and release policy behind these
claims, see [SQL conformance & testing](/docs/conformance).
...
Composite primary and foreign keys, table declarations with multiple unique constraints, and
savepoints are explicit entries in the rejected matrix. These are production-facing gaps, not
forms omitted from the denominator. Scalar, composite, and `UNIQUE` secondary indexes are
supported as Minnow extensions; SQL itself does not standardize physical index DDL. A table still
has one row-addressing primary/unique key, while `CREATE UNIQUE INDEX` adds independently enforced
candidate keys.
