...
keys rather than every key in the table, and an equality-correlated scalar mirrors those key
ranges onto its inner scan as well. Correlated `IN` uses equality-key joins directly or
