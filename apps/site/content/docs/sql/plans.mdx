...
`customer = 42` before the join, through a [secondary index](/docs/sql/ddl#secondary-indexes)
when one exists, or through the unique key when the constant names it. A `LEFT` join keeps its
null-extended rows, which a mirrored predicate would fail, so it gets none.

**Calendar equalities become ranges.** `DATE_TRUNC('month', placed_at) = TIMESTAMP '2025-06-01
00:00:00'` and `EXTRACT(YEAR FROM placed_at) = 2025` are planned as `placed_at >= start AND
placed_at < start + 1 unit`, which skips blocks by value range and runs on the raw datetime
kernel instead of computing a calendar value per row. A timestamp not aligned to its unit can
never equal a truncation, so that predicate is a constant false.
