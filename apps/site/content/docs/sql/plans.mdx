...
path. Correlated scalar aggregates and `IN` use equality-key joins. Forms whose `NULL` semantics
cannot be preserved, such as correlated `NOT IN`, are rejected rather than run row by row — see
