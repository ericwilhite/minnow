...
path, including `EXISTS` nested inside a larger boolean expression. Correlated scalar aggregates
and `IN` use equality-key joins directly or first aggregate by distinct outer probe values for a
range comparison. Correlated `NOT IN` carries explicit empty-set and NULL flags alongside its
anti-join, including for range comparisons. Top-level correlated `ANY` becomes a semi-join on a
true comparison; `ALL` becomes an anti-join on false or unknown comparisons. See the
