...
other select items (`SELECT *, total * 2 AS doubled FROM orders`). With one source, wildcard
outputs use bare column names; with several sources, Minnow uses `alias.column` names so
