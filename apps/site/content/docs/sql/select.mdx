...
Equality-correlated `LATERAL` derived sources are decorrelated into the same set-at-a-time join
path. Correlated lateral queries do not support grouping, `HAVING`, ordering, or `LIMIT`.
...
outer.amount`, decorrelate into semi-joins and anti-joins rather than executing per row.

Correlated scalar aggregates, `IN`, and `NOT IN` require equality correlation. `NOT IN` preserves
its empty-set and NULL behavior during decorrelation. Correlated scalar/`IN` range predicates and
correlated `EXISTS` nested below `OR` remain explicit boundaries rather than per-row execution.
