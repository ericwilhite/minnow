...
outer.amount`, decorrelate into semi-joins and anti-joins rather than executing per row. When a
correlated predicate is nested below `OR`, `NOT`, `CASE`, or a select expression, hidden per-probe
state preserves true, false, and unknown without multiplying outer rows. Nested correlated
`EXISTS`, `IN`, `NOT IN`, `ANY`, `ALL`, and supported scalar blocks use one plan-wide alias
namespace, so sibling branches can combine them to any parser-supported depth. If an `EXISTS`
reaches an outer column only through a nested scalar subquery or another expression, Minnow carries
distinct outer probe tuples into the nested plan before decorrelating it.
...
error for more than one row. Equality and range correlations both remain set-at-a-time: range
forms join distinct outer probes to the inner rows and join the grouped result back by equality.
The inner scalar block cannot itself use `GROUP BY` or `HAVING`.
...
frames. Three limits carry explicit errors: `RANGE` frames take only `UNBOUNDED` and
`CURRENT ROW` bounds, so a numeric offset needs a `ROWS` frame; a window aggregate does not take
`DISTINCT`; and a window function runs only in the select list — to order by one, alias it there
and `ORDER BY` the alias.
