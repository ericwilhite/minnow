...
as `null`. Exact decimals, JSON/JSONB, UUIDs, arrays, `TIME`, intervals, and enums arrive as
strings so no precision or domain value is lost. `columns` describes the shape — name and type per column — so a table component can
