...
as strings so no precision or domain value is lost. `columns` lists the result names, while the
positionally aligned `columnDomains` identifies logical domains such as JSON, UUID, and DATE. A
table component can render headers before it has looked at a single value, and a generic consumer
can parse JSON without query-specific tagging.
