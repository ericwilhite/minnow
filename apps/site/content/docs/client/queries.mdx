...
see [Reading data](/docs/sql/select); the two produce the same plans.
...
The three-argument form covers most filters:
...
| `eb.between(ref, lo, hi)` / `notBetween`    | A range check, with an optional `not`.                        |
...
```ts
// Subquery in a filter: pass the builder itself
...
`db.with(name, factory)` declares a common table expression and returns a client that knows about it:
...
`MATCH` filter plus `BM25` ordering. Minnow keeps the score hidden unless you select it, so your row
...
| `compile()`                 | The internal plan, useful for tooling.    |
| `toSQL()`                   | Parameterized SQL and its bound values.   |
| `live()`                    | A [live query](/docs/client/live).        |

## Escaping to SQL

The `sql` template tag reaches the engine's complete SQL language, with values bound as parameters
rather than pasted into the statement:

```ts
import { sql } from "@minnowdb/client";

const rows = await db.query<{ order_id: number; total: number }>(
  sql`SELECT order_id, total FROM orders WHERE total > ${threshold}`,
);

const result = await db.execute(
  sql`UPDATE orders SET status = ${"review"} WHERE total > ${threshold}`,
);
```

`fragment.execute(db)` remains a short row-query form. Prefer `db.query(fragment)` when you want
query options, and `db.execute(fragment)` for mutations, schema changes, triggers, or transaction
statements. Its result has a `kind` field that tells you exactly what happened.

`executeTakeFirst()` adds a one-row limit before it runs, so it does not fetch a full result only
to discard all but the first row.
