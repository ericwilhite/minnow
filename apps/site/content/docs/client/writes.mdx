...
Both choose rows with a filter and compile to the same update and delete statements as SQL:
...
One builder call is one commit. To make several land together, run them inside a
[write scope](/docs/engine/transactions#atomic-writes) on the driver — every staged mutation
publishes as one atomic commit, and a throw aborts the scope with nothing published.
