...
It ships as its own package so it can version independently of the engine, and it is built only
from the primitives at `@minnowdb/core/plan` — the same ones any other builder would use.
