...
`migrate()` redefines it in place.

Removing a view from your schema does **not** drop it. Nothing distinguishes a view the schema used
to declare from one someone created with `CREATE VIEW`, so dropping the undeclared ones would
destroy the second kind — and "the schema stopped mentioning it" is not proof that it should go.
Drop a view explicitly with `database.dropView(name)`.
