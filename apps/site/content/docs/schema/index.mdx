...
- **`.default(value)` / `.default(fn)`** — fills omitted (or null) slots at insert time. A
  ...
  custom nanoid, anything: the typed client (`insertInto`, `typedTable`) calls it just before
  its parameterized INSERT is sent, so it never persists, never crosses the worker boundary, and write paths
  that skip the typed client don't see it. Defaults require non-nullable columns, and `returning`
