...
- **Everything is async.** Members that are synchronous in the worker return promises on the
  client, and getters become methods: `writer.stats()`, `prepared.memoryUsage()`.
- **`prepareQuery()` still pins its snapshot in the worker** until `close()` — but a client-side
  `execute()` includes a channel round trip. Benchmark inside the worker when isolating executor
  time.
...
clone, so `selectFrom`, typed [writes](/docs/writes/), and `.live()` behave identically.
...
`prepareCacheBytes`. For anything a message can't carry — a custom `BlockStore`, deterministic
...
> durability still ends at a committed transaction. See [Architecture](/docs/architecture/).
