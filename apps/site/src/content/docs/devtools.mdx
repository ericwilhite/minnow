...
and what stands in for it — "Expected VALUES, found SELECT" also explains that `INSERT` takes a
`VALUES` list and cannot read from a query. That comes from the shipped
[feature matrix](/docs/sql/), so it stays true as the engine changes. A message that several
...
panel, in flow, sized by its container. The [playground](/docs/playground/) is exactly this,
