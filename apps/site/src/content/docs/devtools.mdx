...
Both are case-sensitive, and `_` and `%` inside a `contains` value stay live as wildcards — the
engine's `LIKE` has no escape character, so there is nothing to switch them off with.
