...
| SQL        | `query(sql, options?)`, `prepareQuery(sql, options?)`, `explain(sql)`, `execute(sql, params?)`, `runStatement(statement)`. Placeholders (`?`/`$n`) bind through `options.params` or the `execute` parameter list. Statements cover `INSERT ... SELECT`, `ON CONFLICT (key) DO NOTHING / DO UPDATE SET col = EXCLUDED.col`, and `RETURNING` on every mutation. |
