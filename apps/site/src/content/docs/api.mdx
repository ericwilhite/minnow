...
| `.autoIncrement() / .default(value)`                                                                     | Generated values: a persistent cross-tab counter for number unique keys, and `"uuid"` / `"nanoid"` / `"now"` / literal defaults filled at insert time. |
