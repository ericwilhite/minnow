...
From `@minnowdb/core`. See [Typed queries](/docs/queries/) and [Writes](/docs/writes/).
...
| `live()`                                                  | A `LiveQuery<TRow>` over this query. See [Live queries](/docs/live-queries/).                                                                                             |
...
From `@minnowdb/core`. See [Live queries](/docs/live-queries/).
...
`spillOwnerLeaseMs`, `prepareCacheBytes`, and deterministic seams (`now`, `createId`).
`prepareCacheBytes` (default 64 MiB) bounds one shared LRU holding assembled column vectors,
...
| SQL        | `query(sql, options?)`, `snapshot(callback)`, `write(callback)` (atomic multi-mutation scope: every staged batch publishes as one commit, `session.query(sql)` inside the scope reads the pre-scope snapshot plus your own staged writes, and a batch that fails part-way through staging ends the scope — the commit refuses rather than publishing a fragment, even if you catch the error), `explain(sql)`, `execute(sql, params?)`, `runStatement(statement)`. `CREATE TRIGGER name BEFORE\|AFTER INSERT\|UPDATE\|DELETE ON table BEGIN body; ... END` and `DROP TRIGGER name` persist row triggers that fire atomically inside the triggering commit. Bodies use `NEW`/`OLD` references: `INSERT ... VALUES` into keyless tables, `UPDATE`/`DELETE` against keyed tables, and they read everything the commit has staged so far, so repeated firings compound. Triggers fire once per row actually written: deleting a key that does not exist fires nothing, and an upsert fires INSERT triggers for new keys and UPDATE triggers (with the replaced row as `OLD`) for existing ones. Body `INSERT`s may list a subset of columns — the rest fill with defaults or NULL, and a body that omits a non-nullable column without a default is rejected at `CREATE TRIGGER`. Renaming a column a trigger references is rejected by `migrate`. One cascade level is allowed; deeper chains error at write time. Placeholders (`?`/`$n`) bind through `options.params` or the `execute` parameter list. Statements cover `INSERT ... SELECT`, `ON CONFLICT (key) DO NOTHING / DO UPDATE SET col = EXCLUDED.col`, and `RETURNING` on every mutation. |
...
- `PreparedQuery` — synchronous `execute()` over one pinned immutable snapshot; `memoryUsage`,
  `close()`.
...
- Plan tooling — `compileQuery`, `compileStatement`, `executeQuery`, `createPreparedQuery`,
  `optimizePlan`, `renderPlan`, `referencedColumns`, `CompiledQuery`, `CompiledStatement`.
...
From `@minnowdb/core`. See [Workers & multi-tab](/docs/workers/).
...
| `ClientPreparedQuery, ClientBufferedWriter, ClientLiveQuerySet, ClientLiveSubscription` | Handle proxies; synchronous getters become methods (`stats()`, `memoryUsage()`).                                                                         |
...
The checked-in conformance matrix rendered at [SQL support](/docs/sql/): every SQL feature the
