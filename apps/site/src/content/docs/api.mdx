...
| SQL        | `query(sql, options?)`, `snapshot(callback)`, `write(callback)` (atomic multi-mutation scope: every staged batch publishes as one commit, and `session.query(sql)` inside the scope reads your own staged writes), `explain(sql)`, `execute(sql, params?)`, `runStatement(statement)`. `CREATE TRIGGER name BEFORE\|AFTER INSERT\|UPDATE\|DELETE ON table BEGIN body; ... END` and `DROP TRIGGER name` persist row triggers that fire atomically inside the triggering commit. Bodies use `NEW`/`OLD` references: `INSERT ... VALUES` into keyless tables, `UPDATE`/`DELETE` against keyed tables. One cascade level is allowed; deeper chains error at write time. Placeholders (`?`/`$n`) bind through `options.params` or the `execute` parameter list. Statements cover `INSERT ... SELECT`, `ON CONFLICT (key) DO NOTHING / DO UPDATE SET col = EXCLUDED.col`, and `RETURNING` on every mutation. |
