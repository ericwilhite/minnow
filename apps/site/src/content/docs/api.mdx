...
| SQL        | `query(sql, options?)`, `snapshot(callback)`, `write(callback)` (atomic multi-mutation scope: every staged batch publishes as one commit), `explain(sql)`, `execute(sql, params?)`, `runStatement(statement)`. `CREATE TRIGGER name AFTER INSERT\|UPDATE\|DELETE ON table BEGIN insert; ... END` and `DROP TRIGGER name` persist AFTER triggers that fire atomically inside the triggering commit (INSERT ... VALUES bodies over `NEW`/`OLD`, keyless targets, no cascades). Placeholders (`?`/`$n`) bind through `options.params` or the `execute` parameter list. Statements cover `INSERT ... SELECT`, `ON CONFLICT (key) DO NOTHING / DO UPDATE SET col = EXCLUDED.col`, and `RETURNING` on every mutation. |
