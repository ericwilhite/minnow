...
- Each block carries a **checksum** and min/max stats, so queries can skip blocks that can't
  match a filter. There are no user-managed indexes.
...
- **The version-zero block format may change.** Formats only ever grow (new codecs get new IDs;
  old bytes are never reinterpreted), but this early format carries no compatibility promise.
