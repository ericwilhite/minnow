...
- **No database work on the main thread.** The engine runs in a worker; the page holds an async
  proxy. The API is async everywhere, so your code looks the same either way.
...
version N. A commit publishes version N + 1, in strict order:
...
- **Notifications are hints.** BroadcastChannel announces that a new version exists, but every tab
  ...
  [Live queries](/docs/live-queries/) are built on this.
...
- Repeated work is cached at three levels, all inside one byte-bounded budget
  (`prepareCacheBytes`): compiled plans by SQL text, decoded blocks by immutable block id, and
  assembled column vectors by exact visible-segment fingerprint. A commit invalidates the
  fingerprints, but unchanged blocks stay decoded — re-preparing after a small write costs
  re-assembly, not re-fetch and re-decompression.
...
- The [worker protocol](/docs/workers/) is versioned RPC; each handle answers a fixed list of
