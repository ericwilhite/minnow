...
- MinnowDatabase: gzip, 512 KiB target blocks, relaxed IndexedDB durability.
...
- SQL timing: one warm-up plus three measured executions for SQLite, DuckDB, and PGlite through
  each public API. This run predates MinnowDatabase's current `query()`/`prepareQuery()` subset.

MinnowDatabase, SQLite, and PGlite were closed and reopened before their result could pass. All six
...
| Metric                |           MinnowDatabase |  SQLite Wasm 3.53.0 | DuckDB-Wasm 1.5.4 |             PGlite 0.5.4 |
| --------------------- | -----------------------: | ------------------: | ----------------: | -----------------------: |
...
- MinnowDatabase: complete logical IndexedDB payload, including block bytes, key chunks,
...
This historical run did not measure MinnowDatabase SQL. MinnowDatabase now exposes a bounded,
...
| Query median / p95            | MinnowDatabase |      SQLite Wasm |    DuckDB-Wasm |           PGlite |
| ----------------------------- | -------------: | ---------------: | -------------: | ---------------: |
...
MinnowDatabase's previous 100× insert time was 126.55 s at 15,257 rows/s. The optimized run is
...
At this scale MinnowDatabase insert throughput is 1.42× SQLite's and 3.45× PGlite's. In-memory
DuckDB remains 5.23× faster. MinnowDatabase's total adapter time is 1.82× SQLite's speed and is now
...
instead of estimated. MinnowDatabase holds at most one 2.61 MB logical generated batch, encodes one
