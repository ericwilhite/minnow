...
describe("compaction over a long mutation history", () => {
  it.each(seedsFor("compaction-soak", [0x50a4]))(
    "keeps the table equal to a reference across thousands of mutations (seed %s)",
    async (seed) => {
  ...
  it.each(seedsFor("compaction-soak-snapshots", [0x2c17]))(
    "survives compaction interleaved with reads of every earlier state (seed %s)",
    async (seed) => {
