 ...
 * behaviour differs sharply by store in a way that is worth stating rather than discovering.
 *
 * Against the in-memory store, every writer eventually wins: commits settle synchronously enough
 * that the retry loop makes progress and the count of writers does not matter.
 *
 * Against IndexedDB it does matter, and not in the way "retry a few times" suggests. Each writer
 * starts from the same manifest version, and every commit that succeeds invalidates the version
 * every other in-flight writer is holding — so one retry is spent per rival that gets there
 * first. After `maxCommitRetries` rivals have won, everyone still waiting is out of budget at
 * once. The ceiling is therefore `maxCommitRetries + 1` writers, *regardless of how many are
 * queued*: sixty-four concurrent writes do not fail more often than sixteen, they simply leave
 * more losers.
 *
 * These tests pin that shape. They are not an endorsement of it — an application issuing parallel
 * writes to one table in a browser will lose most of them past the ceiling, and the durable fix
 * is to serialize commits rather than let them contend. What the tests guarantee meanwhile is
 * that the losses are *clean*: a rejected write leaves nothing behind, an accepted one is fully
 * present, and the split is deterministic rather than a race.
 */
...
async function contend(
  ...
  const outcomes = await Promise.all(
    Array.from({ length: writers }, (_, index) =>
      database.insertBatch("items", [{ id: 1_000 + index, value: index }]).then(
        () => 1,
        (error: unknown) => {
...
describe("concurrent writes to one table", () => {
  ...
  it("admits exactly maxCommitRetries + 1 writers on IndexedDB, however many are queued", async () => {
