...
/**
 * Dedicated-worker entry that bundles only the in-memory store, published as
 * `@minnowdb/core/worker/memory`. Use it in place of `@minnowdb/core/worker` when the bundler
 ...
 * entry supporting it.
 *
 *   // db-worker.ts: import "@minnowdb/core/worker/memory";
