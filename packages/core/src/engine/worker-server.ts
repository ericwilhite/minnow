 ...
 */
export function singleStoreFactory<Kind extends StoreDescriptor["kind"]>(
  kind: Kind,
  open: (
    descriptor: Extract<StoreDescriptor, { kind: Kind }>,
    options: WorkerStoreOptions,
  ) => BlockStore | Promise<BlockStore>,
): WorkerStoreFactory {
...
function unsupportedStoreKindMessage(bundled: StoreDescriptor["kind"], requested: string): string {
  const requestedLabel = Object.hasOwn(storeKindLabels, requested)
    ? storeKindLabels[requested as StoreDescriptor["kind"]]
    : undefined;
  const requestedEntry =
    requestedLabel === undefined
      ? ""
      : ` or "@minnowdb/core/worker/${requested}" for the ${requestedLabel} store`;
  return (
    `This worker bundles only the ${storeKindLabels[bundled]} store and cannot open ` +
    `${requestedLabel === undefined ? `store kind "${requested}"` : `the ${requestedLabel} store`}; ` +
    `import "@minnowdb/core/worker" for every adapter${requestedEntry}.`
  );
}
