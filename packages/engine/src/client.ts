 ...
 * `@browserdatabase/engine/worker` (or a custom entry built on exposeDatabase()). Construction
...
export interface BrowserDatabaseClientOptions {
  /** Defaults to `{ kind: "indexeddb", name: "browserdatabase" }`. */
...
export class BrowserDatabaseClient {
  ...
  constructor(transport: ClientTransport, options: BrowserDatabaseClientOptions = {}) {
    ...
    const payload: DatabaseInitPayload = {
      store: options.store ?? { kind: "indexeddb", name: "browserdatabase" },
...
export class ClientPreparedQuery {
  constructor(
    private readonly client: BrowserDatabaseClient,
