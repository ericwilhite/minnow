 ...
 * ready-made worker entry (`@browserdatabase/engine/worker`) is one line over the latter.
 *
    ...
    switch (method) {
      ...
      case "prepareQuery": {
        const prepared = await this.database.prepareQuery(
          args[0] as string,
          args[1] as Parameters<BrowserDatabase["prepareQuery"]>[1],
        );
...
async function createServer(
  ...
  const database = new BrowserDatabase(store, payload.options ?? {});
