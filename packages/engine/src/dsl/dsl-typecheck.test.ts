...
function invalidQueriesAreRejected(db: BrowserDb<DB>): void {
...
function derivedAndCteColumnsStayChecked(db: BrowserDb<DB>): void {
  const sub = db
    .selectFrom("orders")
    .groupBy("person")
...
describe("wide-schema type scalability", () => {
  it("keeps precise inference over a 50-table, 22-column database type", () => {
    const db = new BrowserDb<WideDB>(new BrowserDatabase(new MemoryBlockStore()));
    const q = db
