...
async function seeded(): Promise<{ db: BrowserDb<DB>; database: BrowserDatabase }> {
...
describe("live query lifecycle", () => {
  ...
  it("carries cross-tab commit hints through a shared channel name", async () => {
    ...
    const writerDatabase = new BrowserDatabase(store, { rowsPerBlock: 4, compression: "raw" });
    await writerDatabase.migrate(appSchema);
    const writer = new BrowserDb<DB>(writerDatabase, { schema: appSchema });
    await writer.insertInto("people").values({ name: "Ada", score: 1 }).execute();

    const readerDatabase = new BrowserDatabase(store);
    const channelName = `audit-hints-${String(Date.now())}`;
    const reader = new BrowserDb<DB>(readerDatabase, { live: { channelName } });
    const counts: number[] = [];
    ...
    const writerHinted = new BrowserDb<DB>(writerDatabase, {
      schema: appSchema,
      live: { channelName },
    });
