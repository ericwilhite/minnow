...
growing linearly per commit. A 2026-08-12 real-browser measurement (recorded in git
history; the benchmarks page publishes no numbers and runs live in the visitor's browser
instead) confirms the passes end to end: scale-10 dataset inserts 1.85× faster in Chromium
(1.24× Firefox), summed prepares a further 1.45×/1.21× lower, summed execution medians
1.31×/1.27× lower, competitors unchanged within noise, and all 15 reference queries
oracle-verified in both browsers. COALESCE and DATE_TRUNC turned out to be fully implemented
(parser through executor and the SQL feature matrix), so their checklist item is now ticked;
streaming keyed-mutation scan inputs and spilling hash-join build sides remain the open executor
checkbox.
